...
/** Configure a freshly-spawned 'medium' enemy's core stats and progression scaling. */
export function configureMediumSpawn(em: EnemyManager, enemy: Enemy, gameTime: number): void {
  ...
  enemy.speed = 0.92 * 0.30 * baseScale; // was 0.65; ~+41% baseline
  enemy.damage = 7;
  ...
  try { enemy.speed = (em as any).clampToTypeCaps(enemy.speed, 'medium'); } catch {}
  try {
    const ghostCap = 9.0 * ((window as any)?.SPEED_SCALE || 0.45);
