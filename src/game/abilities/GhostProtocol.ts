...
export class GhostProtocolAbility {
  ...
  private end = 0;
  private prevSpeed = 0;
  private lastEnemiesSample: Enemy[] | null = null;
  private lastEnemiesSampleAt = 0;
  // Evolved palette flag (read from player weapons; cached per-frame in draw)
  private evolvedPalette = false;
  // Cached ring textures for performance: [outer, inner]
  private ringTex: Array<{
  ...
  this.player.speed = this.prevSpeed * (mul + (1 - mul) * t);
    ...
    if (now >= this.nextTick) {
      ...
      // Throttle enemy sampling to tick cadence and reuse within draw if needed
      this.lastEnemiesSample = (enemyMgr && typeof enemyMgr.queryEnemies === 'function')
        ? enemyMgr.queryEnemies(this.player.x, this.player.y, r + 16)
        : (enemyMgr?.getEnemies?.() || []);
      this.lastEnemiesSampleAt = now;
      const enemies = this.lastEnemiesSample || [];
  ...
  this.lastEnemiesSample = null;
  this.lastEnemiesSampleAt = 0;
      // Start cooldown
  ...
  ctx.restore();
    ...
    // Text-based AOE rings: cached glyph rings drawn as rotating textures
    ...
    const speeds = ringCount === 1 ? [0.18] : [0.24, -0.18]; // rad/s, opposite directions
    const alphas = ringCount === 1 ? [0.9] : [0.92, 0.64];
    // Build or refresh ring textures (once per ~1s or when params change)
    for (let ri = 0; ri < ringCount; ri++) {
      ...
      const tex = this.ringTex[ri];
      const needsRebuild = !tex || tex.radius !== rr || tex.fontPx !== fontPx || tex.lowFX !== lowFX || tex.evolved !== this.evolvedPalette || tex.cmd !== cmd || (now - tex.timestamp) > 1200;
      if (needsRebuild) {
        this.ringTex[ri] = this.buildRingTexture(rr, fontPx, alpha, this.evolvedPalette, cmd, lowFX);
        this.ringTex[ri]!.timestamp = now;
      }
    }
    // Draw cached rings with rotation
    for (let ri = 0; ri < ringCount; ri++) {
      const rr = radii[ri];
  ...
  // Build an offscreen canvas containing a single glyph ring at radius rr
  private buildRingTexture(rr: number, fontPx: number, alpha: number, evolved: boolean, cmd: string, lowFX: boolean) {
    const canvas = document.createElement('canvas');
    const pad = 6;
    const size = Math.ceil(rr * 2 + pad * 2);
    canvas.width = size;
    canvas.height = size;
    const ctx = canvas.getContext('2d')!;
    ctx.save();
    ctx.translate(size / 2, size / 2);
    // Compute glyph count similar to runtime, but fixed for the texture
    const circ = 2 * Math.PI * rr;
    const glyphW = fontPx * 0.62;
    let count = Math.max(28, Math.min(100, Math.floor(circ / Math.max(6, glyphW + 4))));
    const step = (2 * Math.PI) / count;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.font = `bold ${fontPx}px Orbitron, monospace`;
    const fill = evolved ? `rgba(255,64,64,${alpha.toFixed(3)})` : `rgba(255,190,100,${alpha.toFixed(3)})`;
    ctx.fillStyle = fill as any;
    ctx.shadowColor = evolved ? 'rgba(255,32,32,0.5)' : 'rgba(255,150,0,0.45)';
    ...
    ctx.restore();
    return { canvas, radius: rr, fontPx, lowFX, evolved, cmd, alpha, timestamp: performance.now() };
  }
}
