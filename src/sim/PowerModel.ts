...
function buildOperativePF(operativeId: string, cfg: PowerConfig): PFResult {
  ...
  const shotsPerSec = (60 / Math.max(1, wStats.cooldown)) * ((wStats as any).salvo || 1);
    ...
    if (defWeapon === WeaponType.BIO_TOXIN) {
      ...
      if (scen === 'BOSS') {
        // Conservative average stacks on boss; progressive stacking with decay
        const avgBossStacks = Math.max(2, Math.min(12, 4 + (level - 1) * 0.8));
        ...
        const inSludgeAmp = 1.0; // assume neutral contact on average
