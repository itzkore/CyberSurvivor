/**
 * WebGL2 instanced radial glows renderer (charge glows, simple area glows).
 *
 * - Inputs are world-space (design pixels). Renderer converts to NDC and pixel sizes.
 * - Outputs premultiplied alpha colors; composited with blend ONE, ONE for additive glow.
 */
export interface GLGlowInstance {
...
export class GLGlowsRenderer {
  ...
  constructor(width: number, height: number){
    ...
    this.program = this.createProgram(
      `#version 300 es\n
       ...
       out vec4 vColor;\n
       void main(){\n
         // Scale quad from [-1..1] to pixel offsets of the glow radius.\n
         float px = aPos.x * aRadiusPx;\n
         float py = aPos.y * aRadiusPx;\n
         // Convert pixel offsets to NDC using framebuffer size.\n
      ...
      `#version 300 es\n
       ...
       void main(){\n
         ...
         // Soft radial profile approximating 2D canvas gradient stops: strong center, smooth fade\n
         float core = smoothstep(0.0, 0.25, 1.0 - d);        // inner core\n
         float mid = smoothstep(0.25, 0.7, 1.0 - d) * 0.5;   // mid falloff\n
         float rim = smoothstep(0.7, 1.0, 1.0 - d) * 0.25;   // faint rim\n
         float band = clamp(core + mid + rim, 0.0, 1.0);\n
         float a = vColor.a * band;\n
         if (a <= 0.001) discard;\n
         // Premultiplied output for proper compositing with ONE, ONE blend\n
  ...
  public render(
    ...
    this.ensureCapacity(glows.length);
    ...
    for (let i = 0; i < glows.length; i++){
      const g = glows[i]; if (!g) continue;
      const sx = (g.x - camX);
      const sy = (g.y - camY);
      const xNdc = (sx / designW) * 2 - 1;
      const yNdc = ((sy / designH) * 2 - 1) * -1.0;
      const radiusPx = Math.max(1, g.radius * Math.min(scaleX, scaleY));
      const idx = count * this.strideFloats;
      inst[idx + 0] = xNdc;
      inst[idx + 1] = yNdc;
      inst[idx + 2] = radiusPx;
      inst[idx + 3] = Math.max(0, Math.min(1, g.r));
      inst[idx + 4] = Math.max(0, Math.min(1, g.g));
      inst[idx + 5] = Math.max(0, Math.min(1, g.b));
      inst[idx + 6] = Math.max(0, Math.min(1, g.a));
      count++;
    }
...
export function createGLGlowsRendererLike(mainCanvas: HTMLCanvasElement): GLGlowsRenderer | null {
  try {
    return new GLGlowsRenderer(Math.max(1, mainCanvas.width), Math.max(1, mainCanvas.height));
  } catch {
    return null;
  }
}
